...
def main(logroot):
    ...
    # Exporting logs
    subprocess.check_output(['aws logs create-export-task --region {0} --task-name export-to-s3-task --log-group-name {1} --from {2} --to {3} --destination api-honeypot-logs'.format(region,log_group_name,log_export_start,log_export_stop)], shell=True)
    print("Exporting logs to S3 bucket")
    # Needs to wait at least 60 seconds for export task from CloudWatch to S3 to finish 
    time.sleep(60)
